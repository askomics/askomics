...
class Security(ParamManager):
    ...
    def __init__(self, settings, session, username, email, password, password2):
        ...
        # concatenate salt and password, and hash it with sha256 function
        salted_pw = self.settings["askomics.salt"] + password
    ...
    def check_username_not_in_database(self):
        """
        Check if the username is present in the TS
        """
        self.log.debug('+++ check_username_not_in_database +++')
    ...
    def check_email_not_in_database(self):
        """
        Check if the email is present in the TS
        """
        self.log.debug('+++ check_email_not_in_database +++')
    ...
    def persist_user(self):
        ...
        queryResults = ql.insert_data(chunk, self.settings["askomics.graph"], header_ttl)
