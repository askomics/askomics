...
class AskomicsForceLayoutManager {
  ...
  constructor() {
    ...
    //let svgdiv = $()
    this.vis = d3.select("#svgdiv").append("svg:svg")
...
*/
    ...
    .force("center", d3.forceCenter(this.w/2.0,this.h/2.0))
  ...
  update () {
    ...
    arrow.enter().append("svg:defs").append("svg:marker")
                     ...
                     .attr("orient", "auto")
                     .append("path")
                     .attr("d", "M0,-5L10,0L0,5").merge(arrow);
    ...
    arrow.enter().append("svg:defs").append("svg:marker")
                     ...
                     .attr("orient", "auto")
                     .append("path")
                     .attr("d", "M0,0L10,-5L10,5Z").merge(arrow);
      ...
      link.enter().append("svg:path")
          ...
          .style('stroke', 'grey')
          .style("stroke-dasharray",function(d) {return d.suggested?"2,2":"";})
          .style("opacity", function(d) {return d.suggested?"0.3":"1";})
          .style("stroke-width", "2").merge(link);
      ...
      nodeEnter.append("svg:text")//.append("tspan")
             ...
             .attr("dy","10" )
             .attr("x",14 )
             .text(function (d) { return d.getAttributesWithConstraintsString(); }).merge(node) ;
...
function dragstarted(d) {
            if (!d3.event.active) __ihm.forceLayoutManager.force.alphaTarget(0.3).restart();
            d.fx = d.x;
            d.fy = d.y;
}
...
function dragended(d) {
            if (!d3.event.active) __ihm.forceLayoutManager.force.alphaTarget(0);
            d.fx = null;
            d.fy = null;
}
